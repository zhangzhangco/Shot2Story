...
class Conversation:
    ...
    def append_message(self, role, message, update=True):
        if len(self.messages) > 0 and self.messages[-1][0] == role:
            self.messages[-1][1] = " ".join(
                [self.messages[-1][1].strip(), message.strip()]
            )
        else:
...
def read_video_frames_resized(video_fn, resize_width, resize_height):
    ...
    video_data = np.frombuffer(out, np.uint8).reshape(
        [num_frames, resize_height, resize_width, 3]
    )
...
def get_asr(asr_model, video_fn, fps, shot_list):
    ...
    for shot_cap in shot_list:
        ...
        clip_start = max(
            clip_start - loosen_amount, 0.0
        )  # replace loosen_amount with actual value
        clip_end = min(
            clip_end + loosen_amount, vlen / fps
        )  # replace loosen_amount with actual value
    ...
    for line in shot_lines:
        ...
        try:
            ...
            if start_sec >= end_sec:
                print(
                    f"Invalid input: Start timestamp must be less than end timestamp in '{line}'."
                )
                return True, None
            ...
            print(
                f"Invalid input: Each line should contain exactly two non-negative floats, found '{line}'."
            )
            return True, None
...
def evaluate_shot_detection(total_frames, shot_list, fps):
    ...
    # Define thresholds
    min_shots_threshold = total_frames / (
        30 * fps
    )  # One shot per 30 seconds as an example threshold
...
def get_split(
    ...
    desired_width = 48
    desired_height = 27
    frames_for_shots, fps = read_video_frames_resized(
        video_fn, desired_width, desired_height
    )
    # frames = torch.stack(frames).permute(0,2,3,1)
    frames_for_shots = (
        torch.stack(frames_for_shots).unsqueeze(dim=0).permute(0, 1, 3, 4, 2)
    )
    ...
    if input_splits is not None:
        require_shot_detection, new_splits = evaluate_input_shots(
            input_splits, fps, whole_vlen
        )
    else:
    ...
    if require_shot_detection:
        ...
        if resplit_video:
            ...
            print(
                "New scene detection results",
                len(new_splits),
                len(resplit_video_splits),
            )
    ...
    shot_lengths = [
        (i, end - start, start, end) for i, (start, end) in enumerate(new_splits)
    ]
    ...
    samples = dataset["bdmsvdc_multishot_minigpt_caption"]["20k_val"].get_sample_nfrms(
        len(new_splits), ""
    )
    ...
    shot_frames = []
    for shot_id, split, n_frms in zip(
        samples["shot_ids"], shot_splits, samples["shot_split"]
    ):
        ...
        try:
            ...
            # print(indices)
            print(e)
        assert frms.shape[1] == n_frms, (
            f"{frms.shape}, {len(frms)}, {indices}, {vlen}, {n_frms}"
        )
    ...
    samples["asrs"] = [
        dataset["bdmsvdc_multishot_minigpt_caption"]["20k_val"].text_processor(asr_)
        if len(asr_) != 0
        else ""
        for asr_ in asr
    ]
    samples["video"] = video
    samples["whole_asr"] = dataset["bdmsvdc_multishot_minigpt_caption"][
        "20k_val"
    ].text_processor(whole_asr)
...
class Chat:
    def __init__(self, model, vis_processor, task=None, dataset=None, device="cuda:0"):
        ...
        state_dict = torch.load(
            "/home/zhangxin/Shot2Story/code/pretrain/transnetv2-pytorch-weights.pth"
        )
    ...
    def answer(
        ...
        for output in outputs:
            ...
            output_text = self.model.llama_tokenizer.decode(
                output_token, add_special_tokens=False, skip_special_tokens=True
            )
            # Additional cleanup for special tokens
            for token in ["<unk>", "<s>", "</s>", "<filepath>", ""]:
    ...
    def answer_text(
        ...
        seg_embs = [
            [
                self.model.llama_model.model.embed_tokens(seg_t).squeeze(dim=0)
                for seg_t in seg_bs_t
            ]
            for seg_bs_t in seg_tokens
        ]
    ...
    def upload_video_ms_standalone(
        self, video, conv, img_list, collate_way="none", step=1, input_splits=""
    ):
        ...
        # dataset
        prompt = (
            self.task.get_prompt(self.samples)[0]
            .split("###Human: ")[1]
            .split("###Assistant: ")[0]
        )
    ...
    def upload_video(
        self, video, conv, img_list, collate_way="none", step=1, shot_list=[]
    ):
        if isinstance(video, str):  # is a image path
            if len(shot_list) != 0:
        ...
        if collate_way == "none":
            ...
            # video_emb = video_emb.view((bs, n_frms * video_emb.size(1), video_emb.size(2)))
            img_list.extend(
                v.mean(dim=0).unsqueeze(dim=0) for v in video_emb.split(step, dim=0)
            )
            # for i in range(n_frms):
            #     img_list.append(video_emb[i].unsqueeze(0))
            im_list_str = ""
    ...
    def get_context_emb(self, conv, img_list):
        ...
        assert len(prompt_segs) == len(img_list) + 1, (
            f"Unmatched numbers of image placeholders and images, {prompt} {len(prompt_segs)}, {len(img_list) + 1}."
        )
        seg_tokens = [
            self.model.llama_tokenizer(
    ...
    def prompt_wrap(self, img_embeds, atts_img, prompt):
        ...
        if prompt:
            ...
            mixed_embeds = [
                [
                    torch.cat([seg_e, im_e], dim=0)
                    for im_e, seg_e in zip(img_bs_embs, seg_bs_embs[:-1])
                ]
                + [seg_bs_embs[-1]]
                for img_bs_embs, seg_bs_embs in zip(img_embeds, seg_embs)
            ]
            mixed_embeds = [
                torch.cat(mixed_embeds_, dim=0) for mixed_embeds_ in mixed_embeds
            ]
