...
try:
    ...
    NLGEval = None  # Optional dependency for evaluation
...
class VideoTextPretrainTask(BaseTask):
    def __init__(self, cfg, model_cfg, report_metric=True):
        ...
        for i in range(1, len(self.flexible_sampling_thrs)):
            for f_n in range(
                self.flexible_sampling_thrs[i - 1], self.flexible_sampling_thrs[i]
            ):
                self.flexible_sampling_mapping[f_n] = self.flexible_sampling_frames[
                    i - 1
                ]

        num_frms = self.model_cfg.num_frms
        self.system_prompt = self.model_cfg.get(
            "system_prompt",
            "Given a video, "
        ...
        self.multishot_prompt = self.model_cfg.get(
            "multishot_prompt", "This is a video with {num_shot} shots. "
        )
        self.multishot_prompt = (
            "This is a video with {num_shot} shots. "
            if self.multishot_prompt is None
            else self.multishot_prompt
        )

        self.multishot_secondary_prompt = self.model_cfg.get(
            "multishot_secondary_prompt", "The {shot_idx_text} shot is "
        )
        self.multishot_secondary_prompt = (
            "The {shot_idx_text} shot is "
            if self.multishot_secondary_prompt is None
            else self.multishot_secondary_prompt
        )
        ...
        self.question_prompt = self.model_cfg.get(
            "question_prompt", "Please provide a detailed description of the video."
        )
        self.question_prompt = (
            "" if self.question_prompt is None else str(self.question_prompt)
        )
        ...
        self.inference_prompt = (
            self.prompt_template.format(filted_prompt) + self.answer_prompt
        )
    ...
    def after_evaluation(self, val_result, split_name, epoch, **kwself):
        ...
        if self.report_metric:
            metrics = self._report_metrics(eval_result_file, split_name=split_name)
        else:
    ...
    def _report_metrics(self, eval_result_file, split_name):
        ...
        # print(len(all_ids))
        gt_path = (
            "/mnt/bn/kinetics-lp-maliva-v6/data/hdvila/annotations/{}.json".format(
                split_name
            )
        )
        ...
        for item in annos:
            if self.audio_target:
                ...
                all_gts[item["image_id"]] = [
                    item["whole_caption" if self.whole_video else "caption"]
                ]
    ...
    def get_prompt(self, samples):
        ...
        if self.asr_audio:
            ...
            if self.multishot:
                assert self.whole_video
                prompt = [
                    prompt.format(asr=asr_, promp_img=prompt_prefix) for asr_ in asr
                ]
            else:
                if self.whole_video and not self.fix_total:
                    prompt = [
    ...
    def prompt_wrap(self, model, img_embeds, atts_img, prompt):
        ...
        if prompt:
            ...
            seg_embs = [
                [
                    model.llama_model.model.embed_tokens(seg_t).squeeze(dim=0)
