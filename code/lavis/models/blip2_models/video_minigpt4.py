...
class VideoMiniGPT4(Blip2Base):
    ...
    def __init__(
        ...
        self.multishot_prompt = (
            "This is a video with {num_shot} shots. "
            if multishot_prompt is None
            else str(multishot_prompt)
        )
        self.multishot_secondary_prompt = (
            "The {shot_idx_text} shot is "
            if multishot_secondary_prompt is None
            else str(multishot_secondary_prompt)
        )
        ...
        if self.low_resource:
            ...
            self.llama_model = LlamaForCausalLM.from_pretrained(
                llama_model, torch_dtype=torch.float16, low_cpu_mem_usage=True
            )
        ...
        if prompt_path:
            ...
            if prompt_order == "vt":
                filted_prompts = [
                    "".join([prompt_prefix, prompt]) for prompt in raw_prompts
                ]
            elif prompt_order == "tv":
                filted_prompts = [
                    "".join([prompt, prompt_prefix]) for prompt in raw_prompts
                ]
            ...
            self.prompt_list = [
                system_prompt + prompt_template.format(p) + answer_prompt
                for p in filted_prompts
            ]
    ...
    def prompt_wrap(self, img_embeds, atts_img, prompt):
        if prompt:
            ...
            seg_embs = [
                [
                    self.llama_model.model.embed_tokens(seg_t).squeeze(dim=0)
    ...
    def forward(self, samples):
        ...
        img_embeds = img_embeds.view(
            (bs, n_frms * img_embeds.size(1), img_embeds.size(2))
        )
        ...
        if self.prompt_list:
                ...
                for shot_idx, shot_frms in enumerate(shot_split):
                    ...
                    if len(key) == 1:
                        if key[0] == "shot_idx_text":
                            prompt_prefix += self.multishot_secondary_prompt.format(
                                shot_idx_text=shot_idx_text
                            )
                        else:
                            raise NotImplementedError
                    elif len(key) == 0:
            ...
            if self.asr_audio:
                ...
                if self.multishot:
                    ...
                    prompt_comb = []
                    for asr_, prompt_, cur_mix_multishot_ in zip(
                        asr, prompt, cur_mix_multishot
                    ):
                        if cur_mix_multishot_:
                            ...
                            prompt_comb.append(
                                prompt_.format(
                                    asr=asr_,
                                    shot_idx_text=shot_idx_text,
                                    promp_img=prompt_prefix,
                                )
                            )
                        else:
                            prompt_comb.append(
                                prompt_.format(asr=asr_, promp_img=prompt_prefix)
                            )
                    prompt = prompt_comb
                else:
                    if self.whole_video and not self.fix_total:
                        ...
                        for asr_, prompt_, cur_mix_multishot_ in zip(
                            ...
                            if cur_mix_multishot_:
                                ...
                                prompt_comb.append(
                                    prompt_.format(
                                        asr=asr_, shot_idx_text=shot_idx_text
                                    )
                                )
                            else:
                ...
                if self.multishot:
                    prompt = [
                        prompt_.format(promp_img=prompt_prefix) for prompt_ in prompt
                    ] * bs
        ...
        if self.asr_target:
            ...
            text_target = [
                f"{asr_} {ac_}" for asr_, ac_ in zip(samples["asr"], text_target)
            ]
    ...
    def from_config(cls, cfg):
        ...
        multishot_prompt = cfg.get(
            "multishot_prompt", "This is a video with {num_shot} shots. "
        )
        multishot_secondary_prompt = cfg.get(
            "multishot_secondary_prompt", "The {shot_idx_text} shot is "
        )
        ...
        if ckpt_path:
            ...
            # Validate loading - check for critical missing keys
            ...
            # Check for critical missing keys (llama_proj, Qformer)
            critical_missing = [
                k for k in missing_keys if "llama_proj" in k or "Qformer" in k
            ]
            if critical_missing:
